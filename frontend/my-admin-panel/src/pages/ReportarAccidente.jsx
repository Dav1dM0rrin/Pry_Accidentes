  ...
  const fetchSelectData = async (token) => {
    ...
    try {
      ...
      const [condRes, gravRes, tipoRes, ubiRes] = await Promise.all([
        ...
        axios.get(`${API_URL}/ubicaciones/`, config), // Este endpoint ya debería traer el barrio anidado
      ]);
          ...
          {isLoadingData ? (
            ...
            <form onSubmit={handleSubmit} className="report-form">
              <div className="form-group">
                <label htmlFor="fecha" className="form-label">Fecha y Hora <span className="required-asterisk">*</span>:</label>
                <input type="datetime-local" name="fecha" id="fecha" value={formData.fecha} onChange={handleChange} required className="form-input" disabled={isSubmitting} />
              </div>

              {/* SECCIÓN MODIFICADA PARA UBICACIÓN */}
              <div className="form-group">
                ...
                <select
                  ...
                  disabled={isSubmitting || ubicaciones.length === 0}
                >
                  <option value="">
                    {isLoadingData
                      ? "Cargando..."
                  ...
                  {ubicaciones.map((ubi) =>
                    ubi && typeof ubi.id !== "undefined" ? (
                      <option key={ubi.id} value={ubi.id.toString()}>
                        {`ID Ubic: ${ubi.id} | Barrio: ${
                          ubi.barrio?.nombre || // Accede al nombre del barrio a través del objeto anidado
                          (ubi.barrio_id ? `ID ${ubi.barrio_id} (Nombre no disponible)` : "No asignado")
                        } | Complemento: ${ubi.complemento || "Ninguno"}`}
                      </option>
                    ) : null
                  )}
                </select>
              ...
              <div className="form-group">
                <label htmlFor="tipo_accidente_id" className="form-label">Tipo Accidente <span className="required-asterisk">*</span>:</label>
                <select name="tipo_accidente_id" id="tipo_accidente_id" value={formData.tipo_accidente_id} onChange={handleChange} required className="form-select" disabled={isSubmitting || tiposAccidente.length === 0}>
                  <option value="">{isLoadingData ? "Cargando..." : (tiposAccidente.length === 0 ? "No hay tipos" : "Seleccione tipo")}</option>
                  {tiposAccidente.map(tipo => tipo && typeof tipo.id !== 'undefined' ? (<option key={tipo.id} value={tipo.id.toString()}>{tipo.nombre}</option>) : null)}
                </select>
              </div>
              ...
              <div className="form-grid">
                <div className="form-group">
                  ...
                  <select name="condicion_victima_id" id="condicion_victima_id" value={formData.condicion_victima_id} onChange={handleChange} required className="form-select" disabled={isSubmitting || condicionesVictima.length === 0}>
                    <option value="">{isLoadingData ? "Cargando..." : (condicionesVictima.length === 0 ? "No hay condiciones" : "Seleccione condición")}</option>
                    {condicionesVictima.map(cond => cond && typeof cond.id !== 'undefined' ? (<option key={cond.id} value={cond.id.toString()}>{cond.rol_victima}</option>) : null)}
                  </select>
                </div>
                <div className="form-group">
                  <label htmlFor="gravedad_victima_id" className="form-label">Gravedad Víctima <span className="required-asterisk">*</span>:</label>
                  <select name="gravedad_victima_id" id="gravedad_victima_id" value={formData.gravedad_victima_id} onChange={handleChange} required className="form-select" disabled={isSubmitting || gravedades.length === 0}>
                    <option value="">{isLoadingData ? "Cargando..." : (gravedades.length === 0 ? "No hay gravedades" : "Seleccione gravedad")}</option>
                    {gravedades.map(grav => grav && typeof grav.id !== 'undefined' ? (<option key={grav.id} value={grav.id.toString()}>{grav.nivel_gravedad}</option>) : null)}
                  </select>
                </div>
              </div>
              ...
              <div className="form-grid three-columns">
                <div className="form-group">
                  ...
                  <select name="sexo_victima" id="sexo_victima" value={formData.sexo_victima} onChange={handleChange} className="form-select" disabled={isSubmitting}>
                    <option value="">No especificado</option><option value="M">Masculino</option><option value="F">Femenino</option><option value="O">Otro</option>
                  </select>
                </div>
                <div className="form-group">
                  <label htmlFor="edad_victima" className="form-label">Edad Víctima:</label>
                  <input type="number" name="edad_victima" id="edad_victima" value={formData.edad_victima} onChange={handleChange} min="0" className="form-input" disabled={isSubmitting} placeholder="Ej: 25" />
                </div>
                <div className="form-group">
                  <label htmlFor="cantidad_victima" className="form-label">Cantidad Víctimas:</label>
                  <input type="number" name="cantidad_victima" id="cantidad_victima" value={formData.cantidad_victima} onChange={handleChange} min="1" className="form-input" disabled={isSubmitting} placeholder="Ej: 1" />
                </div>
              </div>
